...
if ( !class_exists( 'spec_options_page' ) ) {
    ...
    class spec_options_page {
        ...
        */
        ...
        function buttons( $options = '' ) { ?>
			...
			<br/>
			<p><?php _e( 'Enabling the following will let people quickly quote from your post. A quote button will pop up next to any text selected that falls inside the CSS selector defined in "Floating quote selector", pressing said button will populate the comment field with the selected text.', SPEC_COMMENT_DOM ); ?></p>
			<p>
			...
			<p>
				<input style="width:98%" class="regular-text" type="text" value="<?php echo esc_attr( $options[ 'quote_target' ] ); ?>" name="<?php $this->item_attrib( 'quote_target', true ); ?>" id="<?php $this->item_attrib( 'quote_target' ); ?>"/>
			</p>
			<p>
				<em><?php _e( 'This should be set to the HTML class or ID of your post content. To find this you will need to view the source code for your posts, search through it for your content and then make a note of the ID or class that is unique to the element that is wrapped around your content. It might look something like this.. &lt;div class="calss-name" id="id-name"&gt;{ Your post content...... }&lt;/div&gt;', SPEC_COMMENT_DOM ) ?></em>
			</p>
        ...
        function titles( $options = '' ) { ?>
			...
			<p>
				<input style="width:98%" class="regular-text" type="text" value="<?php echo esc_attr( $options[ 'title' ] ); ?>" name="<?php $this->item_attrib( 'title', true ); ?>" id="<?php $this->item_attrib( 'title' ); ?>"/>
			</p>

			<p><label for="<?php $this->item_attrib( 'trackback' ); ?>"><?php _e( 'Trackback title', SPEC_COMMENT_DOM ); ?></label></p>
			<p>
				<input style="width:98%" class="regular-text" type="text" value="<?php echo esc_attr( $options[ 'trackback' ] ); ?>" name="<?php $this->item_attrib( 'trackback', true ); ?>" id="<?php $this->item_attrib( 'trackback' ); ?>"/>
			</p>

			<p><label for="<?php $this->item_attrib( 'form_title' ); ?>"><?php _e( 'Comment form title', SPEC_COMMENT_DOM ); ?></label></p>
			<p>
				<input style="width:98%" class="regular-text" type="text" value="<?php echo esc_attr( $options[ 'form_title' ] ); ?>" name="<?php $this->item_attrib( 'form_title', true ); ?>" id="<?php $this->item_attrib( 'form_title' ); ?>"/>
			</p>
        ...
        function stylesheet( $options = '' ) { ?>
			...
			<p><label for="<?php $this->item_attrib( 'stylesheet', true ); ?>"><?php _e( 'Choose the stylesheet you want to use for the comments.', SPEC_COMMENT_DOM ) ?></label>
			</p>
			<p>
        ...
        function credit( $options = '' ) { ?>
			...
			<p><?php _e( 'If you choose to hide our credit link &lsquo;please&rsquo; think about signing up for our newsletter otherwise we get no rewards for our good work.', SPEC_COMMENT_DOM ); ?>
			<img src="<?php echo includes_url( 'images/smilies/icon_cry.gif' ); ?>" alt=":( "/><br/>
			<a href="http://interconnectit.com/">interconnecit<span style="color: #de1301">/</span>it</a></p><?php
        }
        ...
        function comment_nest_depth( $options = '' ) { ?>
			...
			<p>
				<input class="regular-text" type="number" value="<?php echo esc_attr( $options[ 'tc_scroll_offset' ] ); ?>" name="<?php $this->item_attrib( 'tc_scroll_offset', true ); ?>" id="<?php $this->item_attrib( 'tc_scroll_offset' ); ?>"/>
			</p>
            <?php
        }
        ...
        function validate_options( $options = '' ) {
            ...
            if ( isset( $options[ 'reset' ] ) && intval( $options[ 'reset' ] ) == 1 ) {
                ...
                $output[ 'comments_nest_depth' ] = intval( $options[ 'comments_nest_depth' ] ) >= 0 && intval( $options[ 'comments_nest_depth' ] ) <= 10
                    ? intval( $options[ 'comments_nest_depth' ] ) : $this->defaults[ 'comments_nest_depth' ];
                $output[ 'polling' ] = intval( $options[ 'polling' ] ) >= 10 && intval( $options[ 'polling' ] ) <= 999 ? intval( $options[ 'polling' ] )
                    : $this->defaults[ 'polling' ];
                $output[ 'quote_target' ] = preg_match( '/^(\.|#)?[a-zA-Z0-9-_]+$/is', $options[ 'quote_target' ] ) ? $options[ 'quote_target' ]
                    : $this->defaults[ 'quote_target' ];
        ...
        */
        ...
        function add_options_pages() {
            ...
            add_options_page(
                __( 'Spectacu.la Discussion', SPEC_COMMENT_DOM ),
                __( 'Spectacu.la Discussion', SPEC_COMMENT_DOM ),
                'manage_options',
                SPEC_COMMENT_OPT,
        ...
        function do_stuff_boxes() {
            foreach ( ( array )$this->stuff_boxes as $box ) {
                if ( isset( $box[ 'callback' ] ) && is_callable( array( $this, $box[ 'callback' ] ) ) ) { ?>
					<div class="postbox">
						<h3 class="hndle"><?php echo htmlentities( $box[ 'title' ], ENT_QUOTES, get_bloginfo( 'charset' ) ); ?></h3>
						<div class="inside">
				...
				<form method="post" action="options.php" enctype="multipart/form-data">
					...
					<div id="poststuff" class="metabox-holder has-right-sidebar has-sidebar">
						<div id="side-info-column" class="inner-sidebar">
							<div class="postbox">
								<h3 class="hndle"><?php _e( 'Save settings', SPEC_COMMENT_DOM ); ?></label></h3>
								<div class="inside">
									<p>
										<label for="<?php $this->item_attrib( 'reset' ); ?>"><?php _e( 'This will reset all options to their to defaults if checked when saved.', SPEC_COMMENT_DOM ); ?>
    ...
    if ( !function_exists( 'spec_comment_option' ) ) {
        function spec_comment_option( $option_name ) {
            /* @var spec_options_page $spec_options_page ; */
